...
<template>
  <div class="flex flex-col gap-1">
    <label :for="id" class="text-md font-medium text-white font-jura tracking-tight">{{ labelData }}</label>

    <div :id="id" @drop="handleDrop" @dragover="handleDragOver" @dragleave="handleDragLeave" @click="handleClick"
      @focus="handleFocus" @blur="handleBlur" :class="[
      ...
      <div v-else class="text-center">
        <div class="relative inline-block">
          <img :src="previewUrl" alt="Preview" class="max-h-32 max-w-full rounded-lg object-contain" />
        ...
        <button @click="removeFile" class="mt-2 text-sm text-red-400 hover:text-red-300">
          Remove file
        </button>
      </div>
    </div>
